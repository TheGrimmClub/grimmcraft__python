#!/usr/bin/env python3
"""Generate a Python Enum of all vanilla Minecraft (Java Edition) entity types.

Source of truth: PrismarineJS/minecraft-data `entities.json` (one file per version).
This downloads the authoritative list and writes `entity.py` into the package
directory (one level up from this script), containing a `Entity(Enum)` whose
members' .value is the integer registry id, with the .display_name, .type, .category attributes attached as attributes.

Usage:
    python _generate/entities.py               # latest available version
    python _generate/entities.py 1.21          # a specific version
    python _generate/entities.py --list        # print available versions
"""
...
OUTPUT_PATH = SystemPath(__file__).parent.parent / "entity.py"
TYPE_KEY = "entities"
...
def available_versions():
    """Versions (in file order, oldest -> newest) that ship a entities.json."""
...
def build_enum(entries, version):
    ...
    lines = [
        ...
        f"Minecraft Java Edition {version} — {len(entries)} entity types.",
        "",
        ...
        "    category: str",
        "",
        "    def __new__(cls, num_id: int, string_id: str, display_name: str, type: str, category: str) -> Entity:",
    ...
    for e in entries:
        ...
        type = py_str(e.get("type"))
        category = py_str(e.get("category"))
        lines.append(f'    {name} = ({e["id"]}, {py_str(full_id)}, {display_name}, {type}, {category})')
    lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    print(f"Fetching entity list for Java Edition {version} ...")
    entries = fetch_json(data_url(version))
    code = build_enum(entries, version)

    OUTPUT_PATH.write_text(code)
    print(f"Wrote {OUTPUT_PATH} with {len(entries)} entity types. Example: Entity.ZOMBIE")
