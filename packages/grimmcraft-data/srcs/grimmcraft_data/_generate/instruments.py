#!/usr/bin/env python3
"""Generate a Python Enum of all vanilla Minecraft (Java Edition) instrument types.

Source of truth: PrismarineJS/minecraft-data `instruments.json` (one file per version).
This downloads the authoritative list and writes `instrument.py` into the package
directory (one level up from this script), containing a `Instrument(Enum)` whose
members' .value is the integer registry id, with no extra attributes attached as attributes.

Usage:
    python _generate/instruments.py               # latest available version
    python _generate/instruments.py 1.21          # a specific version
    python _generate/instruments.py --list        # print available versions
"""
...
OUTPUT_PATH = SystemPath(__file__).parent.parent / "instrument.py"
TYPE_KEY = "instruments"
...
def available_versions():
    """Versions (in file order, oldest -> newest) that ship a instruments.json."""
...
def build_enum(entries, version):
    ...
    lines = [
        ...
        f"Minecraft Java Edition {version} — {len(entries)} instrument types.",
        "",
...
def main():
    ...
    print(f"Fetching instrument list for Java Edition {version} ...")
    entries = fetch_json(data_url(version))
    code = build_enum(entries, version)

    OUTPUT_PATH.write_text(code)
    print(f"Wrote {OUTPUT_PATH} with {len(entries)} instrument types. Example: Instrument.HARP")
