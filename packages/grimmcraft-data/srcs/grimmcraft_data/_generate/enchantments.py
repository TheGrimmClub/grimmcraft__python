#!/usr/bin/env python3
"""Generate a Python Enum of all vanilla Minecraft (Java Edition) enchantment types.

Source of truth: PrismarineJS/minecraft-data `enchantments.json` (one file per version).
This downloads the authoritative list and writes `enchantment.py` into the package
directory (one level up from this script), containing a `Enchantment(Enum)` whose
members' .value is the integer registry id, with the .display_name, .max_level, .category attributes attached as attributes.

Usage:
    python _generate/enchantments.py               # latest available version
    python _generate/enchantments.py 1.21          # a specific version
    python _generate/enchantments.py --list        # print available versions
"""
...
OUTPUT_PATH = Path(__file__).parent.parent / "enchantment.py"
TYPE_KEY = "enchantments"
...
def available_versions():
    """Versions (in file order, oldest -> newest) that ship a enchantments.json."""
...
def build_enum(entries, version):
    ...
    lines = [
        ...
        f"Minecraft Java Edition {version} — {len(entries)} enchantment types.",
        "",
        ...
        "    def __new__(cls, num_id, string_id, display_name, max_level, category):",
    ...
    for e in entries:
        ...
        max_level = py_str(e.get("maxLevel"))
        category = py_str(e.get("category"))
        lines.append(f'    {name} = ({e["id"]}, {py_str(full_id)}, {display_name}, {max_level}, {category})')
    lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    print(f"Fetching enchantment list for Java Edition {version} ...")
    entries = fetch_json(data_url(version))
    code = build_enum(entries, version)

    OUTPUT_PATH.write_text(code)
    print(f"Wrote {OUTPUT_PATH} with {len(entries)} enchantment types. Example: Enchantment.SHARPNESS")
