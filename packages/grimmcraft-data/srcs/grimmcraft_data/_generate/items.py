#!/usr/bin/env python3
"""Generate a Python Enum of all vanilla Minecraft (Java Edition) item types.

Source of truth: PrismarineJS/minecraft-data `items.json` (one file per version).
This downloads the authoritative list and writes `item.py` into the package
directory (one level up from this script), containing a `Item(Enum)` whose
members' .value is the integer registry id, with the .display_name, .stack_size attributes attached as attributes.

Usage:
    python _generate/items.py               # latest available version
    python _generate/items.py 1.21          # a specific version
...
def available_versions():
    """Versions (in file order, oldest -> newest) that ship a items.json."""
...
def build_enum(entries, version):
    ...
    lines = [
        ...
        f"Minecraft Java Edition {version} — {len(entries)} item types.",
        "",
    ...
    for e in entries:
        ...
        lines.append(f'    {name} = ({e["id"]}, {py_str(full_id)}, {display_name}, {stack_size})')
    lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    print(f"Fetching item list for Java Edition {version} ...")
    entries = fetch_json(data_url(version))
    code = build_enum(entries, version)

    OUTPUT_PATH.write_text(code)
    print(f"Wrote {OUTPUT_PATH} with {len(entries)} item types. Example: Item.OAK_LOG")
