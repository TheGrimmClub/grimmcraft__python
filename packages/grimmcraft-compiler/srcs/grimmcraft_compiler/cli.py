"""The ``grimmcraft-compile`` command-line interface (click + rich)."""
...
# The concrete domain machines the CLI can compile (the grimmcraft demos).
_MACHINE_FACTORIES = {
    "door": "door_machine",
...
def _load_machines(selection: str) -> list[Any]:
    """Build the selected demo machine(s) from ``grimmcraft_control.demos``."""
    from grimmcraft_control import demos

    names = list(_MACHINE_FACTORIES) if selection == "all" else [selection]
    return [getattr(demos, _MACHINE_FACTORIES[name])() for name in names]
